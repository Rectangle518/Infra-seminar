...
__global__ void matrixAdd(const float *a, const float *b, float *c, int M, int N) {
    ...
    int row = /* 填这里 */ blockIdx.y * blockDim.y + threadIdx.y;
    // ====== 空 2：这个线程负责的列号（用 x 方向的内建变量） ======
    int col = /* 填这里 */ blockIdx.x * blockDim.x + threadIdx.x;
    // ====== 空 3：二维边界保护 ======
    if (/* 填这里 */ row < M && col < N) {
...
int main() {
    ...
    dim3 blocks(/* 填这里 */ (N + threads.x - 1) / threads.x, (M + threads.y - 1) / threads.y);
